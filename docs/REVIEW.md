...
| 16 | every keyring check (security review of this pass) | presence of the pinned fingerprint was enough | apt trusts every key in a signed-by= keyring; rpm --import imports every key in the file | exact set of primary fingerprints, on the target, in the hooks and on the build host; a failing keyring is removed |
