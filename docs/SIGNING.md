...
> the image. It is published in [SIGNING-KEY.md](../SIGNING-KEY.md), which
> `package-release` fills in the first time. Never share the private key or its
> passphrase, with anyone. `./build_iso.py gen-key` creates the key and
> `backup-key` backs it up.
